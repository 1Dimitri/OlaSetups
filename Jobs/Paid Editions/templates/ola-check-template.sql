...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Chk - ALL_DATABASES', 
                @enabled=1, 
                ...
                @notify_level_page=0, 
                @delete_level=0, 
                @description=N'Do a full check of all databases using Ola''s solution', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Launch maintenance solution', 
                ...
                @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d !!database_maintenance!! -Q "EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''ALL_DATABASES'',@LogToTable=''Y''" -b', 
                @output_file_name=N'!!LogPath!!\CheckDatabase_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify operator OK', 
                ...
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''!!operator!!'',@subject=N''[!!FriendlyName!!] [!!Instance!!] [OK] DbCheck - ALL_DATABASES'',@body=N''All Databases from !!Instance!! have been checked using DBCC CHECKDB.''
', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify Operator Error', 
                ...
                @retry_interval=0, 
                @os_run_priority=0, @subsystem=N'TSQL', 
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''!!operator!!'',@subject=N''[!!FriendlyName!!] [!!Instance!!] [KO] DbCheck - ALL_DATABASES'',@body=N''Some databases in !!Instance!! failed to pass their DBCC CHECKDB. Please check the CommandLog table in !!database_maintenance!! to know more about the error''
', 
                @database_name=N'!!database_maintenance!!', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Schedule for - DbCheck - ALL_DATABASES', 
                @enabled=1, 
                @freq_type=8, 
                @freq_interval=1, 
                @freq_subday_type=1, 
