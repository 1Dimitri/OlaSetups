...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Launch maintenance solution', 
                ...
                @command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d !!database_maintenance!! -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''ALL_DATABASES'',@LogToTable=''Y''" -b', 
                @output_file_name=N'!!LogPath!!\IndexOptimize_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify operator OK', 
                ...
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''!!operator!!'',@subject=N''[!!FriendlyName!!] [!!Instance!!] [OK] Index - ALL_DATABASES'',@body=N''All Databases from !!Instance!! have their indexes either reorganized or rebuilt.''
', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Notify Operator Error', 
                ...
                @retry_interval=0, 
                @os_run_priority=0, @subsystem=N'TSQL', 
                @command=N'EXECUTE msdb.dbo.sp_notify_operator @name=N''!!operator!!'',@subject=N''[!!FriendlyName!!] [!!Instance!!] [KO] Index - ALL_DATABASES'',@body=N''Some databases in !!Instance!! failed to have their indexes reorganized or rebuilt. Please check the CommandLog table in !!database_maintenance!! to know more about the error''
', 
                @database_name=N'!!database_maintenance!!', 
                @flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
